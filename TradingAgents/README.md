...
The `TradingAgents` engine now ships with a FastAPI wrapper that can be launched independently inside Docker. This setup keeps the NestJS (`server`) and Next.js (`web`) projects running locally while delegating heavy Python execution to a container.

1. Copy `.env.example` to `.env` inside `TradingAgents/` and set `INTERNAL_API_TOKEN`, API keys, and optional overrides such as `TRADINGAGENTS_RESULTS_DIR`.
2. Build the image: `docker compose build trading-agents`.
3. Start the service: `docker compose up trading-agents`.
4. For hot reloads during development, use `docker compose --profile dev up trading-agents-dev`.

The service listens on port `8000` by default. NestJS should target this endpoint via `PYTHON_SERVICE_URL` (e.g., `http://localhost:8000`) while sending the shared `INTERNAL_API_TOKEN` in Bearer authorization headers.
...
### OpenRouter (Embeddings Excluded) + Gemini News Browsing

When selecting OpenRouter as the LLM provider in the CLI, TradingAgents will automatically route news/global-news collection to Gemini web browsing tools (via Google Generative AI) instead of using OpenAI Responses web_search tools, which are not supported by OpenRouter. The agents will still think and reason using your selected OpenRouter models.

Required environment variables:

```bash
# OpenRouter (used only when you select OpenRouter provider)
export OPENROUTER_API_KEY=$YOUR_OPENROUTER_API_KEY
export OPENROUTER_SITE_URL="https://your.site"      # used for policy compliance
export OPENROUTER_APP_TITLE="TradingAgents"

# Gemini browsing for news collection under OpenRouter
export GOOGLE_API_KEY=$YOUR_GOOGLE_API_KEY
```

If `GOOGLE_API_KEY` is not set, the news vendor will fall back to Google/Local sources without Gemini summarization.
